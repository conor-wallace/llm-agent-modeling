...
from stable_baselines3.common.utils import configure_logger, should_collect_more_steps
...
class StaticPolicyAgent(Agent):
    ...
    def get_action(self, obs: np.ndarray, record: bool = True) -> np.ndarray:
        ...
        clipped_actions = clip_actions(actions, self.policy)[0]

        onehot_actions = np.zeros(6)
        onehot_actions[clipped_actions] = 1
...
class OnPolicyAgent(Agent):
    ...
    def __init__(
        ...
        self.model.set_logger(
            configure_logger(self.model.verbose, tensorboard_log, tb_log_name)
        )
    ...
    def get_action(self, obs: Observation, record: bool = True) -> np.ndarray:
        ...
        if record and self.n_steps >= self.model.n_steps:
            buf.compute_returns_and_advantage(
                last_values=self.values, dones=self._last_episode_starts[0]
            )
            ...
            if (
                ...
                self.model.logger.record("name", self.name, exclude="tensorboard")
                ...
                if (
                    ...
                    self.model.logger.record("rollout/ep_rew_mean", safe_mean(rews))
                    self.model.logger.record("rollout/ep_len_mean", safe_mean(lens))
...
class OffPolicyAgent(Agent):
    ...
    def get_action(self, obs: Observation, record: bool = True) -> np.ndarray:
        ...
        if record:
            ...
            if self.old_done:
                ...
                if (
                    self.log_interval is not None
                    and self.model._episode_num % self.log_interval == 0
                ):
                    self.model.logger.record("name", self.name, exclude="tensorboard")
                    self.model.logger.record(
    ...
    def update(self, reward: float, done: bool) -> None:
        ...
        if should_collect_more_steps(
            self.model.train_freq, self.num_collected_steps, self.num_collected_episodes
        ):
            return
        ...
        self.model.train(
            batch_size=self.model.batch_size, gradient_steps=gradient_steps
        )
